...
class Graph:
    ...
    def _match_pattern(self, nodes, pattern):
        """
        Check if a set of nodes match the given pattern, only works for 
        op_name_1 -> op_name_2 -> op_name_3 ...
        """
        pattern_node_op_names = pattern.split("->")
        if len(nodes) != len(pattern_node_op_names):
            return False
        node_ids = [node.id for node in nodes]
        def _dfs_helper(node, node_ids, pattern_node_op_names):
            if node.op_name != pattern_node_op_names[0]:
                return False
            elif node.id not in node_ids:
                return False
            else:
                for node_out in self.outgoing(node):
                    if node_out.id in node_ids:
                        _dfs_helper(node_out, node_ids, pattern_node_op_names[1:])
                return True
            
        for node in nodes:
            if _dfs_helper(node, node_ids, pattern_node_op_names):
                return True
        return False
    ...
    def _post_process_for_quantize_linear(self):
        from .utils import _find_Qlinear_node_info
        quant_linear_nodes = dict()
        for node in self.nodes.values():
            if node.op is None:
                continue
            if type(node.op.module).__name__ == 'QuantizeLinear':
                quant_linear_key = '.'.join(node.param_names[0].split('.')[:-1])
                if quant_linear_key not in quant_linear_nodes:
                    quant_linear_nodes[quant_linear_key] = list()
                quant_linear_nodes[quant_linear_key].append(node)
        
        for key in quant_linear_nodes:
            start_nodes = quant_linear_nodes[key]
            all_param_names = set(sum([start_node.param_names for start_node in start_nodes], []))
            common_parent_node, matmul_node, set_info = _find_Qlinear_node_info(self, start_nodes, parent_node_op_name='add',
                                                                                        matmul_node_op_name="matmul")
            matmul_node.op_name = 'quantizelinear'
            matmul_node.op = start_nodes[0].op
            matmul_node.param_names = list(all_param_names)
            
            nodes_out_common_parent = self.outgoing(common_parent_node)
            
            for node_out_common_parent in nodes_out_common_parent:
                self.add_edge(matmul_node, node_out_common_parent)
            for node_to_remove in set_info:
                if node_to_remove.id == matmul_node.id:
                    continue
    ...
    def _parse_tensors_info(self, state_dict, torch_graph_str):
        ...
        for i, tensor_str in enumerate(tensors_str_list):
            ...
            tensor_type = "input" if i < num_inputs or tensor_id.startswith('input.') or tensor_id.startswith('inputs.') else "params"
    ...
    def random_set_zero_groups(self, target_group_sparsity=None, num_group_divisible=2):
        ...
        for param_group in param_groups:
            ...
            for (p_name, param, p_transform) in zip(param_group['p_names'], param_group['params'], param_group['p_transform']):
                ...
                elif p_transform == TensorTransform.NO_PRUNE:
                    continue
                else:
                    param.data[zero_group_idxes] = 0.0
