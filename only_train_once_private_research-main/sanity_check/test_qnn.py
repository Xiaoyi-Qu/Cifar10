...
class TestQnn(unittest.TestCase):
    def test_sanity(self, dummy_input=torch.rand(1, 3, 32, 32)):
        model = qnn.QMLP(3*32*32, 64, 32, 10)
        oto = OTO(model, dummy_input)
        oto.visualize(view=False, out_dir=OUT_DIR, display_flops=False, display_params=True) # Adjust the arguments if needed.

        # Need to quantize and dequantize the model
